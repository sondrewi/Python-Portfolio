...
# make figure to be used by animation
fig = plt.figure()
ax = plt.axes(xlim=(0, 1), ylim=(-1.2 * 200, 1.2 * 200))
ax.set_xlabel("Point Along Pipe (unit normalised)")
ax.set_ylabel("Air Pressure Deviation from Average (Pa)")
ax.set_title("Animation of Air Pressure along Organ Pipe (Time Range: 5 seconds)")

# define artist object to be rendered
line, = ax.plot([], [], lw=2, color='red')

# create a counter variable that will tell us when animation was quit
counter = 1


def update(i):
    # Function that iteratively fills output array and animates
    ...
    # update animation
    line.set_data(spatial_points, pressure_deviation)
    return line,


# Run update function from time-point 1 to seconds*samplerate
anim = FuncAnimation(fig, update, frames=range(2, timesteps),
                     interval=1, blit=True, repeat=False)

plt.show()

#In case animation is quit early, continue iterations
for i in range(counter, timesteps):
    update(i)
